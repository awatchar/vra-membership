...
การย่อตามสัดส่วนและ re-encode รูปขนาดใหญ่ทำที่ browser ส่วน Worker เป็นฝ่าย **ตรวจ** ไม่ใช่เชื่อ: อ่านขนาดพิกเซลจาก container header (ไม่ต้องมี decoder) ตรวจขนาดสูงสุดและลบ metadata segment ทิ้ง Worker ไม่บังคับสัดส่วนรูปตามการตัดสินใจใน #52 ภาพที่ผู้สมัครอัปโหลดเองต้องมีอย่างน้อย 300 × 400 พิกเซล แต่ field `face` จาก iApp ใช้ขนาดที่ provider คืนมาได้โดยไม่บังคับขั้นต่ำ เพราะการ upscale ไม่ได้เพิ่มรายละเอียดและจะทำให้ผู้สมัครติดอยู่หลังยืนยัน preview แล้ว ข้อยกเว้นนี้ไม่เปลี่ยนเพดานขนาด ชนิด JPEG การตรวจ magic bytes หรือการลบ metadata และ Worker ไม่มี canvas; การใส่ WASM decoder จะเพิ่ม bundle size กับ CPU ทุก upload โดยไม่คุ้มที่ปริมาณ 1-2 ใบสมัครต่อวัน
...
SlipOK อาจตอบ HTTP 400/code `1014` พร้อมข้อมูลธุรกรรมที่ตรวจสอบแล้ว เมื่อบัญชีผู้รับไม่ตรงกับบัญชีที่ผูกไว้กับ branch ของ SlipOK ข้อมูลนี้ยังไม่ใช่การอนุมัติ payment: adapter ส่งเฉพาะโมเดลธุรกรรมภายในต่อให้ service ทำข้อ 2–5 ด้วยค่า `VRA_BANK_ACCOUNT` ฝั่ง server ตามปกติ ถ้าข้อมูลไม่ครบหรือบัญชีไม่ตรงจริง ระบบยัง fail closed ส่วน fallback ที่ต้องส่งภาพใช้ชื่อกลางซึ่งมีนามสกุลตรงกับ content type เพราะ SlipOK ใช้นามสกุลเลือก image decoder โดยไม่ส่งชื่อไฟล์เดิมของผู้สมัครออกไป
...
ทั้ง wizard ผู้สมัครและ portal ผู้จัดการใช้ header กับ footer ชุดเดียวกัน เพราะเป็นสมาคมเดียวกัน และผู้ใช้ไม่ควรรู้สึกว่าเป็นสองระบบ

เหตุผลที่มีสองอันนี้ไม่ใช่ความสวยงาม: คนที่กดลิงก์เข้ามาจะกรอกเลขบัตรประชาชนและโอนเงิน และก่อนหน้านี้สิ่งเดียวที่บอกว่าเป็นระบบของใครคือ `<title>` ของ tab สำหรับหน้าที่ขอข้อมูลระดับนั้น **การบอกว่าตัวเองเป็นใครเป็นคุณสมบัติด้านความปลอดภัย ไม่ใช่การตกแต่ง** — เป็นสิ่งที่คนตรวจก่อนตัดสินใจว่าจะเชื่อฟอร์มนี้หรือไม่

footer มีที่อยู่ตู้ ปณ. อีเมล LINE ID และเบอร์ผู้จัดการ เพราะผู้สมัครที่ติดอยู่ระหว่างโอนเงินกับเห็นหน้ายืนยัน — จุดที่ต้องการช่องทางติดต่อมากที่สุด — ก่อนหน้านี้ไม่มีทางติดต่อใครได้เลย

`alt` ของโลโก้มีข้อความจริง ไม่ใช่ `""` เพราะมันไม่ใช่ภาพประดับ: เป็นหนึ่งในสองสิ่งบนหน้าที่บอกว่าเป็นสมาคมใด ผู้ใช้ screen reader จึงต้องได้ยินชื่อมัน

### ค่าติดต่อมาจาก configuration

ค่าทั้งห้าอยู่ใน `vars` ของ `wrangler.jsonc` ทั้งสอง environment ไม่ใช่ secret เพราะไม่มีอะไรต้องปิด — เจ้าของยืนยันแล้วว่าข้อมูลติดต่อของสมาคมเป็นข้อมูลสาธารณะ (#21) การเก็บเป็น var ที่ commit ไว้ทำให้ค่าตั้งต้นถูก review ได้ และการแก้ใน Cloudflare dashboard มีผลทันทีโดยไม่ต้องแก้โค้ด — เบอร์โทรที่เปลี่ยนไม่ควรต้องเปิด PR

**ค่าที่ไม่ได้ตั้งจะไม่ขึ้นบรรทัดนั้นเลย** ไม่ขึ้นแถวว่างและไม่ขึ้นค่าตัวอย่าง เพราะค่าตัวอย่างที่ดูเหมือนจริงจะมีคนลองใช้ ส่วนชื่อสมาคมมีค่า fallback คงที่ เพราะหน้าที่บอกว่าไม่ได้เป็นของใครแม้ชั่วครู่แย่กว่าหน้าที่กลาง ๆ ชั่วครู่

`/api/config` เป็น endpoint สาธารณะไม่ต้องผ่าน Access ดังนั้น header กับ footer ถูกต้องแม้บนหน้าที่บอกว่ายืนยันสิทธิ์ผู้จัดการไม่ได้ — ซึ่งเป็นหน้าที่ต้องการช่องทางติดต่อมากที่สุด

### ขนาดไฟล์โลโก้

ต้นฉบับ 500×500 สองไฟล์อยู่ใน `assets/brand/` เป็นแหล่งที่มา ไม่ใช่ไฟล์ที่เสิร์ฟ สิ่งที่ client ส่งจริงคือ `src/web/assets/vra-logo.png` ขนาด 192px 24 KB ซึ่งย่อมาด้วย `scripts/resize-png.ts`

192 พิกเซลเลือกมาโดยตั้งใจ: โลโก้แสดงที่ 40–48 CSS pixel จึงรองรับจอ 4× ได้โดยไม่ต้องส่ง byte ที่ไม่มีใครเห็น ต้นฉบับ 96 KB จะกลายเป็น asset ชิ้นใหญ่ที่สุดบนหน้าที่โหลด bundle 137 KB — เพื่อภาพที่เล็กกว่า thumbnail

`scripts/resize-png.ts` เขียนเองด้วย `zlib` ที่มากับ Node ไม่ได้เพิ่ม dependency เพราะมันรันครั้งเดียวเพื่อสร้าง asset ที่ commit ไว้ การเพิ่ม native image library เข้า devDependencies เพื่องานครั้งเดียวแลกไม่คุ้มกับโค้ดหกสิบบรรทัด สคริปต์ยัง drop chunk `eXIf` และ `iTXt` ที่ต้นฉบับมีติดมาด้วย — ทั้งสองไม่ควรอยู่ในไฟล์ที่เสิร์ฟให้ผู้เข้าชมทุกคน
