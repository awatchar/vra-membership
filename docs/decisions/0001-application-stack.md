...
3. **Client: React + Vite** build ลง `dist/client` แล้วให้ Workers assets binding เสิร์ฟ ขั้นตอนสมัครสมาชิกเป็น wizard หลายขั้นที่มีการเลือกและย่อภาพกับ decode QR ฝั่ง browser ซึ่งต้องการ state management ที่ตรวจสอบได้ การเขียนด้วย DOM API ตรง ๆ จะกลายเป็น state machine ที่ทำเองและ test ยากกว่า (ข้อกำหนด crop เดิมถูกแทนที่ด้วยการเก็บรูปทั้งเฟรมใน #52 โดยไม่กระทบการเลือก stack)
